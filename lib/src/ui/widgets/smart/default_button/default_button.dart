...
import 'package:plantagoda/src/ui/widgets/smart/default_button/default_buton_shape.dart';
import 'package:plantagoda/src/ui/widgets/smart/default_button/default_button_position.dart';

import 'default_buton_size.dart';
import 'default_button_type.dart';

class DefaultButton extends StatefulWidget {
  /// Create buttons of all types. check out [DefaultIconButton] for icon buttons, and [DefaultBadge] for badges
  const DefaultButton({
    Key key,
    ...
    this.child,
    this.type = DefaultButtonType.solid,
    this.shape = DefaultButtonShape.standard,
    this.color,
    this.textColor,
    this.position = DefaultPosition.start,
    this.size = DefaultSize.MEDIUM,
    this.borderSide,
    this.text,
    this.icon,
    this.blockButton,
  ...
  /// The box shadow for the button's [Material], if DefaultButtonType is solid
  final BoxShadow boxShadow;

  /// Pass [DefaultColors] or [Color]. The color for the button's [Material] when it has the input focus.
  final Color focusColor;

  /// Pass [DefaultColors] or [Color]. The color for the button's [Material] when a pointer is hovering over it.
  final Color hoverColor;

  /// Pass [DefaultColors] or [Color]. The highlight color for the button's [InkWell].
  final Color highlightColor;

  /// Pass [DefaultColors] or [Color]. The splash color for the button's [InkWell].
  ...
  /// Button type of [DefaultButtonType] i.e, solid, outline, outline2x, transparent
  final DefaultButtonType type;

  /// Button type of [DefaultButtonShape] i.e, standard, pills, square, shadow, icons
  final DefaultButtonShape shape;

  /// Pass [DefaultColors] or [Color]
  final Color color;
  ...
  /// size of [double] or [DefaultSize] i.e, 1.2, small, medium, large etc.
  final double size;
  ...
  /// icon type of [DefaultPosition] i.e, start, end
  ...
  /// on true state default box shadow appears around button, if DefaultButtonType is solid
...
class _DefaultButtonState extends State<DefaultButton> {
  ...
  Widget build(BuildContext context) {
    ...
    Color getBorderColor() {
      if (widget.enabled) {
        final Color fillColor = color ?? Theme.of(context).primaryColor;
        if (fillColor != null) {
          return fillColor;
        }
      } else {
    ...
    Color getDisabledFillColor() {
      if (widget.type == DefaultButtonType.transparent ||
          widget.type == DefaultButtonType.outline ||
          widget.type == DefaultButtonType.outline2x) {
    ...
    Color getDisabledTextColor() {
      ...
      } else if (widget.type == DefaultButtonType.outline ||
          widget.type == DefaultButtonType.outline2x ||
          widget.type == DefaultButtonType.transparent) {
        return color;
      } else {
        return Color(0xff222428);
      }
    }

    Color getTextColor() {
      if (widget.type == DefaultButtonType.outline ||
          widget.type == DefaultButtonType.outline2x ||
          widget.type == DefaultButtonType.transparent) {
    ...
    final BorderSide shapeBorder = widget.type == DefaultButtonType.outline ||
            widget.type == DefaultButtonType.outline2x
        ? outlineBorder
        : widget.borderSide ??
            BorderSide(
    ...
    if (shape == DefaultButtonShape.pills) {
    ...
    } else if (shape == DefaultButtonShape.square) {
      shapeBorderType = RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(0),
        side: shapeBorder,
      );
    } else if (shape == DefaultButtonShape.standard) {
      shapeBorderType = RoundedRectangleBorder(
      ...
      if (widget.type != DefaultButtonType.transparent) {
        if (widget.boxShadow == null && widget.buttonBoxShadow != true) {
          ...
          return BoxDecoration(
            color: widget.type == DefaultButtonType.transparent ||
                    widget.type == DefaultButtonType.outline ||
                    widget.type == DefaultButtonType.outline2x
                ? Colors.transparent
                : color,
            borderRadius: widget.shape == DefaultButtonShape.pills
                ? BorderRadius.circular(50)
                : widget.shape == DefaultButtonShape.standard
    ...
    TextStyle getTextStyle() {
      if (widget.size == DefaultSize.SMALL) {
        return TextStyle(
          color: widget.enabled ? getTextColor() : getDisabledTextColor(),
          fontSize: 12,
        );
      } else if (widget.size == DefaultSize.MEDIUM) {
        return TextStyle(
          color: widget.enabled ? getTextColor() : getDisabledTextColor(),
          fontSize: 13,
          fontWeight: FontWeight.w400,
        );
      } else if (widget.size == DefaultSize.LARGE) {
        return TextStyle(
          color: widget.enabled ? getTextColor() : getDisabledTextColor(),
          fontSize: 14,
    ...
    final Widget result = Container(
      ...
      decoration:
          widget.type == DefaultButtonType.solid ? getBoxShadow() : null,
      child: Material(
        elevation: _effectiveElevation,
        textStyle: widget.textStyle == null ? getTextStyle() : widget.textStyle,
        shape: widget.type == DefaultButtonType.transparent
            ? null
        ...
        child: InkWell(
          ...
          customBorder: widget.type == DefaultButtonType.transparent
              ? null
              : widget.borderShape ?? shapeBorderType,
          child: IconTheme.merge(
            ...
            child: Container(
              ...
              child: Center(
                ...
                heightFactor: 1,
                child: icon != null &&
                        (position == DefaultPosition.start || position == null)
                    ? Row(
                    ...
                    : icon != null && (position == DefaultPosition.end)
                        ? Row(
                            mainAxisSize: MainAxisSize.min,
                            children: <Widget>[
                              child,
